...
librarysize<- ggplot(data=df, aes(x=Index, y=LibrarySize, color=Sample_or_Control)) + geom_point()+ theme_classic()+
  theme(axis.title.x = element_text(size=14))  +
  theme(axis.text.x = element_text(size=12))+
  theme(axis.title.y=element_text(size=14))+
  theme(axis.text.y=element_text(size=12))+
  theme(legend.text=element_text(size=14))+
  theme(legend.title=element_text(size=16))##Supplemental Figure S2
librarysize
#Figure S1
ggsave(filename = "librarySize_04.08.25.tiff", plot = librarysize,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)

ggsave(filename = "librarySize_04.08.25.pdf", plot = librarysize,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)
...
prevalence<- ggplot(data=df.pa, aes(x=pa.neg, y=pa.pos, color=contaminant)) + geom_point() +theme_classic()+
  theme(axis.title.x = element_text(size=14))  +
  theme(axis.text.x = element_text(size=12))+
  theme(axis.title.y=element_text(size=14))+
  theme(axis.text.y=element_text(size=12))+
...
prevalence
#Figure S1
ggsave(filename = "prevalence_04.08.25.tiff", plot = prevalence,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)

ggsave(filename = "prevalence_04.08.25.pdf", plot = prevalence,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)
...
data_rarecurve<-t(as.data.frame(otu_table(phyloseq_merged1)))
any(rowSums(data_rarecurve[])<1)
data_rarecurve_nozero = data_rarecurve[rowSums(data_rarecurve[])>0, ,drop=FALSE]
rarefaction<- rarecurve(data_rarecurve_nozero, label = FALSE, col="cyan4", step=200) ## Supplemental Figure S7
rarefaction_plot<-map_dfr(rarefaction, bind_rows) %>% 
  bind_cols(Group=rownames(data_rarecurve_nozero),.) %>%
  pivot_longer(-Group) %>% 
  drop_na()%>%
  mutate(n_seqs=as.numeric(str_replace(name, "N", "")))%>%
  select(-name)%>%
  ggplot(aes(x=n_seqs, y=value, group=Group))+
  geom_vline(xintercept=12000, color="gray")+
  geom_line()+
  theme_classic()+
  xlab("Sample size")+
  ylab("Species")+
...
rarefaction_plot
#Figure S1
ggsave(filename = "rarefaction_04.08.25.tiff", plot = rarefaction_plot,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)
ggsave(filename = "rarefaction_04.08.25.pdf", plot = rarefaction_plot,
       width = 17,
       height = 15, units = c("cm"),
       dpi = 300)
...
library(vegan)
library(dplyr)
rarecurve(t(otu_table(phyloseq_merged)), step=20, label = FALSE, col="cyan4")
...
#Figure 3
ggsave(filename = "barplot_class_allsamples_hormone_resus_ABA_SA_08.19.25.pdf", figure, ##CHANGED FILE NAMES AS NEEDED
       width = 160,
       height = 100, units = c("cm"),
       dpi = 300, limitsize = FALSE)
...
phyloseq_class_test1<- phyloseq_class %>% filter(Class=="c__Bacilli" & Treatment_new=="Abscisic-acid" & Timepoint=="1-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test1)

Kruskal-Wallis rank sum test

data:  Abundance by Plant
Kruskal-Wallis chi-squared = 9.3633, df = 1, p-value = 0.002214


###Verrucomicrobiae ABA day 1
##bean vs sw
phyloseq_class_test2<- phyloseq_class %>% filter(Class=="c__Verrucomicrobiae" & Treatment_new=="Abscisic-acid" & Timepoint=="1-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test2)

Kruskal-Wallis rank sum test

data:  Abundance by Plant
Kruskal-Wallis chi-squared = 4.7143, df = 1, p-value = 0.02991


##Myxococcia ABA day 1
#bean vs sw
phyloseq_class_test3<- phyloseq_class %>% filter(Class=="c__Myxococcia" & Treatment_new=="Abscisic-acid" & Timepoint=="1-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test3)

Kruskal-Wallis rank sum test

data:  Abundance by Plant
Kruskal-Wallis chi-squared = 17.28, df = 1, p-value = 3.226e-05


##Actinobacteria ABA day 1
##bean vs sw
phyloseq_class_test4<- phyloseq_class %>% filter(Class=="c__Actinobacteria" & Treatment_new=="Abscisic-acid" & Timepoint=="1-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test4)
...
##Alphaproteobacteria ABA 7 day 
##bean vs switchgrass

phyloseq_class_test5<- phyloseq_class %>% filter(Class=="c__Alphaproteobacteria" & Treatment_new=="Abscisic-acid" & Timepoint=="7-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test5)

	Kruskal-Wallis rank sum test

data:  Abundance by Plant
Kruskal-Wallis chi-squared = 7.0533, df = 1, p-value = 0.007912


##Alphaproteobacteria ABA 14 day
##bean vs sw
phyloseq_class_test6<- phyloseq_class %>% filter(Class=="c__Alphaproteobacteria" & Treatment_new=="Abscisic-acid" & Timepoint=="14-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test6)

Kruskal-Wallis rank sum test

data:  Abundance by Plant
Kruskal-Wallis chi-squared = 5.1856, df = 1, p-value = 0.02277



##Actinobateria SA Day 1
##bean vs sw

phyloseq_class_test7<- phyloseq_class %>% filter(Class=="c__Actinobacteria" & Treatment_new=="Salicylic-acid" & Timepoint=="1-Day")
kruskal.test(Abundance~Plant, data=phyloseq_class_test7)
...
metadata<- metadata %>% filter(Plant=="Switchgrass", Treatment!="Abscisic-acid")##change the plant and tretament for bean and SW aba and sa plots

metadata<- read.csv("sample-metadata-edaphic-constrained.csv")
metadata<- metadata %>% filter(Plant=="Switchgrass", Treatment!="Salicylic-acid")

metadata<- read.csv("sample-metadata-edaphic-constrained.csv")
metadata<- metadata %>% filter(Plant=="Bean", Treatment!="Abscisic-acid")

metadata<- read.csv("sample-metadata-edaphic-constrained.csv")
metadata<- metadata %>% filter(Plant=="Bean", Treatment!="Salicylic-acid")
...
pcoa_bean_aba<-plot_ordination(
  physeq = phyloseq_merged_final,
  ordination = cap_ord,
  color = "Treatment",
  shape = "Timepoint",
  title = "Bean Abscisic Acid (ABA)"
) + theme_classic()+
  geom_point(aes(color = Treatment, shape=Timepoint, size=Timepoint), alpha=0.4) +
...
pcoa_sw_aba<-plot_ordination(
  physeq = phyloseq_merged_final,
  ordination = cap_ord,
  color = "Treatment",
  shape = "Timepoint",
  title = "Switchgrass Abscisic Acid (ABA)"
) + theme_classic()+
  geom_point(aes(color = Treatment, shape=Timepoint, size=Timepoint), alpha=0.4) +
  scale_shape_manual(values=c(16,16,16,16), breaks=c('PreHormone', '1-Day', '7-Day', '14-Day'))+
  scale_color_manual(name="Treatment", limits = c("Field-Soil (PreHormone)", "Predry (PreHormone)", "Postdry (PreHormone)", "Water-acclimated (PreHormone)", "Abscisic-acid", "Methanol-control", "Water-control"), values = c("#000000", "#E69F00", "#56B4E9", "#009E73", 
...
pcoa_sw_sa<-plot_ordination(
  ...
  color = "Treatment",
  shape = "Timepoint",
  title = "Switchgrass Salicylic Acid (SA)"
) + theme_classic()+
  geom_point(aes(color = Treatment, shape=Timepoint, size=Timepoint), alpha=0.4) +
  scale_shape_manual(values=c(16,16,16,16), breaks=c('PreHormone', '1-Day', '7-Day', '14-Day'))+
  scale_color_manual(name="Treatment", limits = c("Field-Soil (PreHormone)", "Predry (PreHormone)", "Postdry (PreHormone)", "Water-acclimated (PreHormone)", "Salicylic-acid", "Methanol-control", "Water-control"), values = c("#000000", "#E69F00", "#56B4E9", "#009E73", 
...
ggsave(filename="pcoa-bean-aba-withOUT-mesocosmeffect-constrained-08.19.25.TIFF", plot=pcoa_bean_aba, width=6.8, height=6, units="in", dpi=720)
ggsave(filename="pcoa-bean-sa-withOUT-mesocosmeffect-constrained-08.19.25.TIFF", plot=pcoa_bean_sa, width=6.8, height=6, units="in", dpi=720)
ggsave(filename="pcoa-sw-aba-withOUT-mesocosmeffect-constrained-08.19.25.TIFF", plot=pcoa_sw_aba, width=6.8, height=6, units="in", dpi=720)
ggsave(filename="pcoa-sw-sa-withOUT-mesocosmeffect-constrained-08.19.25.TIFF", plot=pcoa_sw_sa, width=6.8, height=6, units="in", dpi=720)



##merging all pcoas with A, B, C, D
figure_pcoa = ggpubr::ggarrange(pcoa_bean_aba, pcoa_bean_sa,
                                pcoa_sw_aba, pcoa_sw_sa,
                           labels = c("A", "B", "C", "D"),
                           ncol = 2, nrow = 2,
                           common.legend = F,
                           legend = c("right"))
...
figure_pcoa
#figure_pcoa_1<- ggpubr::ggpar(figure_pcoa, font.legend = c(size = 12, "plain", "black"))
#figure_pcoa_1

#Figure 4
ggsave(filename="pcoa-all_04.07.25.PDF", plot=figure_pcoa, width=13, height=8, units="in", dpi=300)
ggsave(filename="pcoa-all_08.19.25.PDF", plot=figure_pcoa, width=13, height=8, units="in", dpi=300)
...
metadata<- read.csv("deseq_bean_aba_labelchange.csv") 
metadata<- read.csv("deseq_bean_sa_labelchange.csv") 
metadata<- read.csv("deseq_sw_sa_labelchange.csv") 
metadata<- read.csv("deseq_sw_aba_labelchange.csv") 
View(metadata)
...
plot_bean_watercontrol<- ggplot(sigtab, aes(x=Genus, y=log2FoldChange, color=Phylum)) + 
  #scale_x_reordered() +
...
ggsave(filename = "Deseq2-bean-watercontrol.tiff", plot = plot_bean_watercontrol, ##CHANGED NAMES AS NEEDED. 
       width = 20,
       height = 15, units = c("cm"),
       dpi = 300)

#Figure S2
##similarly saved for Deseq2-bean-aba.tiff, Deseq2-bean-sa.tiff, Deseq2-bean-methanol.tiff, Deseq2-sw-aba.tiff,
#Deseq2-sw-sa.tiff, Deseq2-sw-methanol.tiff
...
##new with deseq otus (Figures S3)
...
##methanol controls
bean_met<- read.csv("mat_data_merge_bean_met_con_01.28.25_withtax.csv")
sw_met<- read.csv("mat_data_merge_sw_met_con_01.28.25_withtax.csv")
...
#methanol 
mat_data_merge_size_beanmet<- bean_met%>% group_by(ASV, Treatment_Timepoint) %>% mutate(count=sum(Abund_max_standard>0)/n()) %>% mutate(mean=mean(Abund_max_standard)) %>% mutate(inactive_NA=ifelse(all(dna==0 & cdna==0), NA, mean))
mat_data_merge_size_swmet<- sw_met%>% group_by(ASV, Treatment_Timepoint) %>% mutate(count=sum(Abund_max_standard>0)/n()) %>% mutate(mean=mean(Abund_max_standard)) %>% mutate(inactive_NA=ifelse(all(dna==0 & cdna==0), NA, mean))
...
#methanol
mat_data_merge_size_beanmet<- mat_data_merge_size_beanmet %>% mutate(grid="Bean Methanol")
mat_data_merge_size_swmet<- mat_data_merge_size_swmet %>% mutate(grid="Switchgrass Methanol")

##for aggregated bean , sw , aba, sa

bb<-c(0.820,0.600,0.400,0.200,0.00001) # define breaks.
lb<-c(1,0.8,0.6,0.4,0.2,0.08)
ll<-c("1","0.8","0.6","0.4","0.2","0.08") # labels.

##for methanol bean/sw 
bb<-c(0.820,0.600,0.400,0.200,0.00005) # define breaks.
lb<-c(1,0.8,0.6,0.4,0.2,0.0001)
ll<-c("1","0.8","0.6","0.4","0.2","0.0001") # labels.
...
mat_data_merge_size_swmet$Treatment_Timepoint_new<- recode(mat_data_merge_size_swmet$Treatment_Timepoint, "01 - Field Soil" = "Field Soil",
                                                     "02 - Predry" ="Predry",
                                                     "03 - Postdry"="Postdry",
                                                     "04 - Water acclimated" = "Water acclimated",
                                                     "05 - Methanol control (1D)" = "Methanol control (1D)",
                                                     "9 - Methanol control (7D)" = "Methanol control (7D)",
                                                     "13 - Methanol control (14D)" = "Methanol control (14D)")

##bean methanol
mat_data_merge_size_beanmet$Treatment_Timepoint_new<- recode(mat_data_merge_size_beanmet$Treatment_Timepoint, "01 - Field Soil" = "Field Soil",
                                                     "02 - Predry" ="Predry",
                                                     "03 - Postdry"="Postdry",
                                                     "04 - Water acclimated" = "Water acclimated",
                                                     "05 - Methanol control (1D)" = "Methanol control (1D)",
...
mat_data_merge_size_beanmet$Treatment_Timepoint_new<- factor(mat_data_merge_size_beanmet$Treatment_Timepoint_new, levels=c("Field Soil", "Predry", "Postdry", "Water acclimated", "Methanol control (1D)","Methanol control (7D)", "Methanol control (14D)"), ordered=TRUE)
mat_data_merge_size_swmet$Treatment_Timepoint_new<- factor(mat_data_merge_size_swmet$Treatment_Timepoint_new, levels=c("Field Soil", "Predry", "Postdry", "Water acclimated", "Methanol control (1D)","Methanol control (7D)", "Methanol control (14D)"), ordered=TRUE)


#merging for the combined aggregate plot
...
##for methanol only bean
mat_data_merge_size_beanmet_color<- mat_data_merge_size_beanmet %>% mutate(color=ifelse(OTUclass %in% deseq, "red", "black"))
mat_data_merge_size_beanmet_color$color<- factor(mat_data_merge_size_beanmet_color$color, levels=c("red", "black"), ordered=TRUE)
mat_data_merge_size_beanmet_color<- mat_data_merge_size_beanmet_color %>% arrange(color)

# **CRITICAL:  Force the OTUclass factor order**
mat_data_merge_size_beanmet_color$OTUclass <- factor(mat_data_merge_size_beanmet_color$OTUclass, levels = unique(mat_data_merge_size_beanmet_color$OTUclass), ordered = TRUE)

#for methanol only sw
mat_data_merge_size_swmet_color<- mat_data_merge_size_swmet %>% mutate(color=ifelse(OTUclass %in% deseq, "red", "black"))
mat_data_merge_size_swmet_color$color<- factor(mat_data_merge_size_swmet_color$color, levels=c("red", "black"), ordered=TRUE)
mat_data_merge_size_swmet_color<- mat_data_merge_size_swmet_color %>% arrange(color)

# **CRITICAL:  Force the OTUclass factor order**
mat_data_merge_size_swmet_color$OTUclass <- factor(mat_data_merge_size_swmet_color$OTUclass, levels = unique(mat_data_merge_size_swmet_color$OTUclass), ordered = TRUE)
...
#new label function for aggregated plot
...
#new label function for separate methanol plot
# Create the labels using a *lookup* – much more efficient and correct
label_function <- function(otu_class) {
  color_lookup <- setNames(mat_data_merge_size_beanmet_color$color, mat_data_merge_size_beanmet_color$OTUclass) # Named vector for lookup
  paste0("<span style='color:", color_lookup[otu_class], ";'>", otu_class, "</span>")
}
label_function <- function(otu_class) {
  color_lookup <- setNames(mat_data_merge_size_swmet_color$color, mat_data_merge_size_swmet_color$OTUclass) # Named vector for lookup
  paste0("<span style='color:", color_lookup[otu_class], ";'>", otu_class, "</span>")
}
...
##bubble plot all (bean, sw, aba, sa)
...
##bubble plot SW ABA
plot<- ggplot(mat_data_merge_size_swaba_color,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_swaba, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
...
plot<- ggplot(mat_data_merge_size_swsa_color,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_swsa, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
  scale_colour_viridis(limits=c(0.00005,0.820),breaks=bb)+
...
plot<- ggplot(mat_data_merge_size_swmet_color,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_swmet, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
  scale_colour_viridis(limits=c(0.00005,0.820),breaks=bb)+
...
plot<- ggplot(mat_data_merge_size_beanaba,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+ ##NO DESEQ RESULTS FOR THIS SO USING THE mat_data_merge_size data frame
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_beanaba, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
  scale_colour_viridis(limits=c(0.00001,0.820),breaks=bb)+
...
##BEAN SA
plot<- ggplot(mat_data_merge_size_beansa_color,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_beansa, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
  scale_colour_viridis(limits=c(0.00005,0.820),breaks=bb)+
...
##bean methanol
plot<- ggplot(mat_data_merge_size_beanmet_color,aes(x=Treatment_Timepoint_new,y=OTUclass,size=count,color=inactive_NA))+
  geom_point()+
  geom_point(data=subset(mat_data_merge_size_beanmet, is.na(inactive_NA)), aes(shape='NA'), size=3) +
  scale_shape_manual(values=c('NA'=3), name=NULL, labels="Not detected in DNA or cDNA") +
  #theme(strip.text.x=element_text(size=10))+
  #geom_text(aes(label = OTUclass, color = color), vjust = -0.5) +
  scale_colour_viridis(limits=c(0.00005,0.820),breaks=bb)+
...
##switchgrass
#Figure S4
ggsave(filename = "sw_met_con_top50_aggregated_04.10.25.tiff", plot = plot,
       width = 30,
       height = 27, units = c("cm"),
       dpi = 300)
ggsave(filename = "sw_met_con_top50_aggregated_04.10.25.pdf", plot = plot,
       width = 30,
       height = 27, units = c("cm"),
       dpi = 300)

##other plot saving code for separate visualization
...
##bean 
#Figure S4
ggsave(filename = "bean_met_con_top50_aggregated_04.10.25.tiff", plot = plot,
       width = 30,
       height = 27, units = c("cm"),
       dpi = 300)
ggsave(filename = "bean_met_con_top50_aggregated_04.10.25.pdf", plot = plot,
       width = 30,
       height = 27, units = c("cm"),
       dpi = 300)
...
cap_sw<- data %>% filter(Plant=="Switchgrass", pH!="NA") #Nucleic_Acid=="RNA")
...
data<- read.csv("sample-metadata-edaphic-constrained.csv", row.names=1)
cap_bean<- data %>% filter(Plant=="Bean", pH!="NA") #Nucleic_Acid=="RNA")
...
sample_data() Sample Data:       [ 50 samples by 15 sample variables ]
tax_table()   Taxonomy Table:    [ 8887 taxa by 7 taxonomic ranks ]
...
cap_plot<- ggplot(data=cap.ord.df, aes(x=CAP1, y=CAP2)) +
  geom_point(aes(color=Treatment, shape=Timepoint), size=5)+
...
cap.plot.sw = cap_plot + 
  ...
  geom_label(mapping = label_map, data = filter(arrowdf, xend < 0), show.legend = FALSE, size=4, hjust=1, fill="white", color="black") +
  geom_label(mapping = label_map, data = filter(arrowdf, xend > 0), show.legend = FALSE, size=4, hjust=0, fill="white", color="black") +
  labs(title = paste("Variables explain ", round(100*RsquareAdj(cap_ord)$r.squared, 3), "% of the \ncommunity variation in Switchgrass", sep="")) + xlim(-2, 2)+
...
cap.plot.bean = cap_plot + 
  geom_segment(mapping = arrow_map, linewidth = 1.5, data = arrowdf, color = "black", arrow = arrowhead) + 
  geom_segment(mapping = arrow_map, linewidth = 0.5, data = arrowdf, color = "white", arrow = arrowhead) + 
  geom_label(mapping = label_map, data = filter(arrowdf, xend < 0), show.legend = FALSE, size=4, hjust=1, fill="white", color="black") +
  geom_label(mapping = label_map, data = filter(arrowdf, xend > 0), show.legend = FALSE, size=4, hjust=0, fill="white", color="black") +
  labs(title = paste("Variables explain ", round(100*RsquareAdj(cap_ord)$r.squared, 3), "% of the \ncommunity variation in Bean", sep="")) + xlim(-2, 2)+
  theme_classic() + labs(shape="Timepoint", color="Treatment")+
  theme(plot.title=element_text(hjust=0.5, face="bold", size=16),
        plot.subtitle =element_text(hjust=0.5, face="bold", size=16))+
  theme(legend.text=element_text(size=12),
...
figure_cap = ggpubr::ggarrange(cap.plot.bean, cap.plot.sw,
                                labels = c("A", "B"),
                                ncol = 2, nrow = 1,
                                common.legend = T,
                                legend = c("right"))
...
ggsave(filename = "cap_bean_04.08.25_bray.TIFF", plot = cap.plot.bean,
       width = 17,
       height = 13, units = c("cm"),
       dpi = 300)
ggsave(filename = "cap_sw_04.08.25_bray.TIFF", plot = cap.plot.sw,
       width = 17,
       height = 13, units = c("cm"),
       dpi = 300)

#Figure S5
ggsave(filename = "cap_bean_sw_04.08.25_bray.TIFF", plot = figure_cap,
       width = 34,
       height = 13, units = c("cm"),
       dpi = 300)
ggsave(filename = "cap_bean_sw_04.08.25_bray.pdf", plot = figure_cap,
       width = 34,
       height = 13, units = c("cm"),
       dpi = 300)
